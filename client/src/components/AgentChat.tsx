      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {historyLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-text-muted">
            <Bot size={40} className="mb-3 text-text-muted" />
            <p className="text-sm">Send a message to start chatting with {agentName}</p>
            <p className="text-xs text-text-muted mt-1">The agent will respond based on their SOUL personality and current tasks</p>
          </div>
        ) : (
          <>
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
        isUser ? 'bg-brand-accent/20 text-brand-accent' : 'bg-teal-500/20 text-teal-400'
      }`}>
