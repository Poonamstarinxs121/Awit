...
export function Dock() {
  ...
  const dockW = expanded ? DOCK_EXPANDED_W : DOCK_COLLAPSED_W;
  ...
  return (
    <aside style={{
      ...
      zIndex: 50, transition: 'width 220ms cubic-bezier(0.4,0,0.2,1)',
      overflow: 'visible',
    }}>

      {/* Expand/collapse tab — right edge, vertically centered */}
      <button
            ...
            <div key={section.label} style={{ marginBottom: '2px' }}>
              {si > 0 && (
                <div style={{
                  ...
                  margin: expanded ? '6px 14px 6px' : '5px 12px 5px',
                }} />
              )}
              ...
              {/* Items */}
              {(expanded ? isOpen : true) && section.items.map(item => {
      ...
      <div ref={menuRef} style={{
        ...
        <button
          ...
          style={{
            width: '100%', display: 'flex', alignItems: 'center',
            justifyContent: expanded ? 'flex-start' : 'center',
            gap: '10px', padding: expanded ? '6px 14px' : '8px 0',
        ...
        {showUserMenu && (
          <div style={{
            ...
            left: expanded ? '8px' : `${DOCK_COLLAPSED_W + 8}px`,
            width: '240px',
        ...
        {showThemePicker && (
          <div style={{
            position: 'absolute', bottom: 'calc(100% + 8px)',
            left: expanded ? '256px' : `${DOCK_COLLAPSED_W + 256}px`,
            width: '210px',
            backgroundColor: 'var(--card)', border: '1px solid var(--border)',
            borderRadius: '14px', boxShadow: '0 16px 48px rgba(0,0,0,0.7)',
