      ...
      <div className="flex-1 flex flex-col min-w-0">
        {globalPaused && (
          <div className="bg-amber-50 border-b border-amber-200 px-4 py-1.5 text-center">
            <span className="text-amber-700 text-xs font-medium">All agents paused — heartbeats and cron jobs suspended.</span>
          </div>
        )}
        <header className="h-14 border-b border-border-default bg-white flex items-center justify-between px-6 shrink-0">
          <div className="flex items-center gap-3">
            {pageTitle && (
              <h2 className="text-sm font-semibold text-text-primary font-heading">{pageTitle}</h2>
            )}
            {user?.tenantName && (
              ...
              <>
                ...
                >
                  {globalPaused ? (
                    ...
                    <>
                      <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M5.75 3a.75.75 0 00-.75.75v12.5c0 .414.336.75.75.75h1.5a.75.75 0 00.75-.75V3.75A.75.75 0 007.25 3h-1.5zM12.75 3a.75.75 0 00-.75.75v12.5c0 .414.336.75.75.75h1.5a.75.75 0 00.75-.75V3.75a.75.75 0 00-.75-.75h-1.5z" /></svg>
                      Pause All
                    </>
                  )}
                </button>
